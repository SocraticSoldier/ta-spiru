...
const PRODUCTS: readonly ProductSeed[] = [
  { sku: 'TS-WAX-MATT', name: 'Matt Pomade', brand: "Ta' Spiru Cosmetics", category: ProductCategory.STYLING_WAX, priceCents: 1450, ledgerTag: LedgerTag.RETAIL_BARBER },
  { sku: 'TS-WAX-CEMENT', name: 'Cement Wax', brand: "Ta' Spiru Cosmetics", category: ProductCategory.STYLING_WAX, priceCents: 1450, ledgerTag: LedgerTag.RETAIL_BARBER },
  { sku: 'TS-SHMP-250', name: 'Daily Shampoo 250ml', brand: "Ta' Spiru Cosmetics", category: ProductCategory.HAIR_CARE, priceCents: 1200, ledgerTag: LedgerTag.RETAIL_BARBER },
  { sku: 'TS-BEARD-OIL', name: 'Beard Oil', brand: "Ta' Spiru Cosmetics", category: ProductCategory.SHAVE_BEARD, priceCents: 1600, ledgerTag: LedgerTag.RETAIL_BARBER },
  { sku: 'WS-SNOWFOAM-1L', name: 'Snow Foam 1L', brand: 'Work Stuff', category: ProductCategory.DETAILING_CHEMICAL, priceCents: 1900, ledgerTag: LedgerTag.RETAIL_CAR_CARE },
  { sku: 'GS-IRON-GEL', name: 'Iron Remover GEL', brand: 'Good Stuff', category: ProductCategory.DETAILING_CHEMICAL, priceCents: 2200, ledgerTag: LedgerTag.RETAIL_CAR_CARE },
  { sku: 'WS-DRY-TOWEL', name: 'Drying Towel XL', brand: 'Work Stuff', category: ProductCategory.DETAILING_HARDWARE, priceCents: 2500, ledgerTag: LedgerTag.RETAIL_CAR_CARE },
];
...
const main = async (): Promise<void> => {
  ...
  for (const spec of PRODUCTS) {
    const product = await prisma.product.upsert({
      where: { sku: spec.sku },
      update: { priceCents: spec.priceCents },
      create: { ...spec },
    });
    for (const { id: locationId } of locations) {
